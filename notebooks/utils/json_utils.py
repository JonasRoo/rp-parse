...
def dump_json_to_file(
    data: dict, folder_name: str = ".", file_name: str = "data.json", **kwargs
) -> None:
    """
...
def merge_all_json_in_folder(
    dir_name: str,
    merged_file_name: str = "merged_data.json",
        ...
        with open(f, "r", encoding="utf-8") as input_file:
            ...
            if isinstance(file_data, list):
                for json_object in file_data:
                    current_data.append(json_object)
            else:
                current_data.append(file_data)
            if prepend_filename:
                data.append({f: current_data})
            else:
                data.append(data)
